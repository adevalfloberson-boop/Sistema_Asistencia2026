...
<head>
    ...
    @if (($activePage ?? null) === 'devices')
        <meta name="device-status-url" content="{{ route('devices.statuses') }}">
    @endif
...
<body class="min-h-screen overflow-x-hidden bg-stone-50 font-sans text-slate-900 antialiased transition-colors dark:bg-slate-950 dark:text-slate-100">
    @php
        ...
        $canManageSchool = $canManageSchool ?? false;
        $dashboardBaseRoute = $dashboardBaseRoute ?? 'dashboard.admin';
        $dashboardPageRoute = $dashboardPageRoute ?? 'dashboard.admin.page';
    ...
    <div class="min-h-screen lg:grid lg:grid-cols-[17.5rem_minmax(0,1fr)]">
        <aside class="hidden border-r border-teal-900/70 bg-teal-950 text-teal-50 lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col">
            <div class="border-b border-white/10 px-6 py-6">
                <a class="flex items-center gap-3" href="{{ route($dashboardBaseRoute) }}">
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-6 text-sm">
                @foreach ([
                    ...
                    <a class="group flex items-center gap-3 rounded-xl px-3 py-3 font-semibold text-teal-100/70 transition hover:bg-white/10 hover:text-white {{ $activePage === $anchor ? 'bg-white/10 text-white' : '' }}" href="{{ $anchor === 'overview' ? route($dashboardBaseRoute) : route($dashboardPageRoute, $anchor === 'reports' ? 'attendance' : $anchor) }}">
            ...
            <div class="p-4">
                <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    <p class="mt-1 text-xs leading-5 text-teal-100/55">Los lectores ADMS reportan directamente al sistema.</p>
                </div>
            </div>
        </aside>
        ...
        <div class="min-w-0">
            <header class="sticky top-0 z-30 border-b border-stone-200/80 bg-stone-50/90 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/85">
                ...
                <nav class="no-scrollbar flex gap-1 overflow-x-auto border-t border-stone-200/70 px-4 py-2 text-xs font-bold lg:hidden dark:border-slate-800">
                    @foreach ([['overview', 'Resumen'], ['devices', 'Lectores'], ['enrollment', 'Huellas'], ['students', 'Estudiantes'], ['attendance', 'En vivo']] as [$anchor, $label])
                        <a class="whitespace-nowrap rounded-lg px-3 py-2 text-slate-500 hover:bg-teal-50 hover:text-teal-700 dark:text-slate-400 dark:hover:bg-teal-950" href="{{ $anchor === 'overview' ? route($dashboardBaseRoute) : route($dashboardPageRoute, $anchor) }}">{{ $label }}</a>
                    @endforeach
                </nav>
            </header>
            ...
            <main class="mx-auto max-w-[1500px] space-y-8 px-5 py-7 sm:px-8 sm:py-9">
                ...
                @if ($activePage === 'overview')
                    <form class="grid gap-3 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:grid-cols-2 lg:grid-cols-4" method="GET" action="{{ route($dashboardBaseRoute) }}">
                        <label class="text-xs font-bold text-slate-500">Fecha<input class="mt-2 w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-950" type="date" name="date" value="{{ $analysisDate->format('Y-m-d') }}"></label>
                        <label class="text-xs font-bold text-slate-500">Escuela<select class="mt-2 w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-950" name="school_id"><option value="">Todas</option>@foreach ($schools as $school)<option value="{{ $school->id }}" @selected($selectedSchoolId === $school->id)>{{ $school->name }}</option>@endforeach</select></label>
                        <label class="text-xs font-bold text-slate-500">Curso<select class="mt-2 w-full rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-950" name="course"><option value="">Todos</option>@foreach ($allCourses->where('is_active', true)->pluck('name')->unique() as $courseName)<option value="{{ $courseName }}" @selected($selectedCourse === $courseName)>{{ $courseName }}</option>@endforeach</select></label>
                        <div class="flex items-end gap-2"><button class="flex-1 rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-black text-white" type="submit">Analizar</button><a class="rounded-xl border border-stone-200 px-4 py-2.5 text-sm font-bold dark:border-slate-700" href="{{ route($dashboardBaseRoute) }}">Limpiar</a></div>
                    </form>
                @endif
                ...
                <section id="overview" class="scroll-mt-32 overflow-hidden rounded-3xl bg-teal-900 text-white shadow-xl shadow-teal-950/10 dark:bg-teal-950">
                    <div class="relative grid gap-8 px-6 py-8 sm:px-9 lg:grid-cols-[1.3fr_.7fr] lg:items-end lg:px-10 lg:py-10">
                        ...
                        <div class="relative"><span class="inline-flex items-center gap-2 rounded-full bg-amber-300 px-3 py-1.5 text-[11px] font-black uppercase tracking-[.16em] text-teal-950"><span class="h-2 w-2 rounded-full bg-teal-700"></span>Jornada escolar en curso</span><h1 class="mt-5 max-w-2xl text-3xl font-black leading-tight tracking-tight sm:text-4xl">{{ $isSuperadmin ? 'Red global de lectores y asistencia.' : 'Una vista clara de la asistencia de tu centro.' }}</h1><p class="mt-3 max-w-2xl text-sm leading-6 text-teal-100/75">{{ $isSuperadmin ? 'Supervisa centros, lectores ADMS y reglas operativas desde una consola técnica.' : 'Administra estudiantes, cursos, docentes y lectores asignados a tu institución.' }}</p></div>
                ...
                <section id="devices" class="scroll-mt-32 space-y-5">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p class="text-xs font-black uppercase tracking-[.18em] text-teal-700 dark:text-teal-300">Centro de operaciones</p><h2 class="mt-1 text-2xl font-black tracking-tight">Red global de lectores</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Una vista central, como la consola de un ISP, para supervisar cada escuela y lector.</p></div><div class="flex gap-2"><span class="rounded-full bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-300"><span data-device-summary-online>{{ $deviceSummary['online'] }}</span>&nbsp;en línea</span><span class="rounded-full bg-rose-100 px-3 py-2 text-xs font-bold text-rose-700 dark:bg-rose-400/15 dark:text-rose-300"><span data-device-summary-alerts>{{ $deviceSummary['offline'] }}</span>&nbsp;sin comunicación</span></div></div>

                    <div id="device-connection-alert" class="hidden items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-900 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-100" role="status"><span class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-amber-400 font-black text-amber-950">!</span><span data-device-alert-message></span></div>
                ...
                <section id="attendance" class="scroll-mt-32 overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"><div class="flex flex-col gap-3 border-b border-stone-100 px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6 dark:border-slate-800"><div><p class="text-xs font-black uppercase tracking-[.18em] text-teal-700 dark:text-teal-300">En tiempo real</p><h2 class="mt-1 text-xl font-black">Actividad reciente</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Lecturas recibidas hoy desde los dispositivos.</p></div><span class="inline-flex w-fit items-center gap-2 rounded-full bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-300"><span class="h-2 w-2 animate-pulse rounded-full bg-emerald-500"></span>Captura activa</span></div><div class="overflow-x-auto"><table class="w-full min-w-[900px] text-left text-sm"><thead class="bg-stone-50 text-xs uppercase tracking-wider text-slate-500 dark:bg-slate-950 dark:text-slate-400"><tr><th class="px-6 py-4">Estudiante</th><th class="px-6 py-4">Matrícula</th><th class="px-6 py-4">Curso</th><th class="px-6 py-4">Lector / escuela</th><th class="px-6 py-4">Hora</th><th class="px-6 py-4">Registro</th><th class="px-6 py-4">Observación</th></tr></thead><tbody class="divide-y divide-stone-100 dark:divide-slate-800">@forelse ($ultimosRegistros as $registro)<tr class="hover:bg-stone-50 dark:hover:bg-slate-800/50"><td class="px-6 py-4 font-bold">{{ $registro['nombre'] }}</td><td class="px-6 py-4 font-mono text-xs text-slate-500">{{ $registro['matricula'] }}</td><td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ $registro['curso'] }}</td><td class="px-6 py-4"><span class="font-bold">{{ $registro['lector'] }}</span>@if ($registro['escuela'])<span class="mt-1 block text-xs text-slate-500">{{ $registro['escuela'] }}</span>@endif</td><td class="px-6 py-4 font-mono font-bold text-teal-700 dark:text-teal-300">{{ $registro['hora'] }}</td><td class="px-6 py-4"><span class="rounded-full px-3 py-1 text-xs font-bold {{ $registro['tipo'] === 'Entrada' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-400/15 dark:text-emerald-300' : 'bg-amber-100 text-amber-700 dark:bg-amber-400/15 dark:text-amber-300' }}">{{ $registro['tipo'] }}</span></td><td class="px-6 py-4">@if ($registro['excuse_type'])<span class="text-xs font-black text-sky-700 dark:text-sky-300">Justificada</span><p class="mt-1 max-w-44 text-xs text-slate-500">{{ $registro['excuse_note'] }}</p>@elseif ($registro['is_late'] || $registro['is_early_departure'])<details><summary class="cursor-pointer text-xs font-black text-rose-600">{{ $registro['is_late'] ? 'Tardanza' : 'Salida temprana' }}</summary><form class="mt-2 grid w-56 gap-2" action="{{ route('attendances.excuses.store', $registro['id']) }}" method="POST">@csrf<input type="hidden" name="excuse_type" value="{{ $registro['is_late'] ? 'late' : 'early_departure' }}"><textarea class="rounded-lg border border-stone-200 bg-stone-50 p-2 text-xs dark:border-slate-700 dark:bg-slate-950" name="excuse_note" required placeholder="Motivo de la excusa"></textarea><button class="rounded-lg bg-teal-700 px-3 py-2 text-xs font-black text-white" type="submit">Justificar</button></form></details>@else<span class="text-xs font-bold text-slate-400">Sin alertas</span>@endif</td></tr>@empty<tr><td class="px-6 py-14 text-center text-slate-500" colspan="7">Aún no se han recibido registros hoy.</td></tr>@endforelse</tbody></table></div></section>
