...
<section id="teachers" class="{{ $cardClass }}">
    <div class="border-b border-stone-100 px-5 py-5 sm:px-6 dark:border-slate-800"><p class="text-xs font-black uppercase tracking-[.18em] text-teal-700 dark:text-teal-300">Equipo del centro</p><h2 class="mt-1 text-xl font-black">Usuarios, roles y cursos</h2></div>
    <form class="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4 sm:p-6" action="{{ route('teachers.store') }}" method="POST">@csrf
        <label class="text-sm font-bold"><span class="mb-2 block">Escuela</span><select class="{{ $fieldClass }}" name="school_id" required>@foreach ($schools as $school)<option value="{{ $school->id }}">{{ $school->name }}</option>@endforeach</select></label>
        <label class="text-sm font-bold"><span class="mb-2 block">Rol</span><select class="{{ $fieldClass }}" name="role"><option value="teacher">Docente · pasa lista</option><option value="viewer">Visualización · solo consulta</option><option value="admin">Admin · un centro</option></select></label>
        ...
        <fieldset class="text-sm font-bold sm:col-span-2">
            <legend class="mb-2">Cursos asignados</legend>
            <div class="grid max-h-52 gap-2 overflow-y-auto rounded-xl border border-stone-200 bg-stone-50 p-3 dark:border-slate-700 dark:bg-slate-950 sm:grid-cols-2">
                @foreach ($allCourses->where('is_active', true) as $course)
                    <label class="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 font-semibold hover:bg-white dark:hover:bg-slate-900">
                        <input class="h-4 w-4 rounded border-stone-300 text-teal-700 focus:ring-teal-500" name="course_ids[]" type="checkbox" value="{{ $course->id }}" @checked(in_array($course->id, old('course_ids', [])))>
                        <span>{{ $course->school?->code }} · {{ $course->name }}</span>
                    </label>
                @endforeach
            </div>
            <p class="mt-2 text-xs font-normal text-slate-500">Marca uno o varios cursos.</p>
        </fieldset>
                ...
                <details class="mt-3">
                    ...
                    <form class="mt-3 grid grid-cols-2 gap-2" action="{{ route('teachers.update', $teacher) }}" method="POST">
                        ...
                        <fieldset class="col-span-2">
                            <legend class="mb-2 text-sm font-bold">Cursos asignados</legend>
                            <div class="grid max-h-48 gap-2 overflow-y-auto rounded-xl border border-stone-200 bg-stone-50 p-3 dark:border-slate-700 dark:bg-slate-950 sm:grid-cols-2">
                                @foreach ($allCourses->where('school_id', $teacher->school_id)->where('is_active', true) as $course)
                                    <label class="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 text-sm font-semibold hover:bg-white dark:hover:bg-slate-900">
                                        <input class="h-4 w-4 rounded border-stone-300 text-teal-700 focus:ring-teal-500" name="course_ids[]" type="checkbox" value="{{ $course->id }}" @checked($teacher->courses->contains($course))>
...
<section id="settings" class="{{ $cardClass }}">
    <div class="border-b border-stone-100 px-5 py-5 sm:px-6 dark:border-slate-800"><p class="text-xs font-black uppercase tracking-[.18em] text-teal-700 dark:text-teal-300">Reglas del plantel</p><h2 class="mt-1 text-xl font-black">Horario, tardanzas y salidas</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">El primer ponche es entrada; al pasar el intervalo configurado, el siguiente será salida. Las salidas antes de la hora programada quedan señaladas, sin bloquearse.</p></div>
    <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">@foreach ($schools as $school)<form class="rounded-xl border border-stone-200 p-4 dark:border-slate-700" action="{{ route('schools.settings.update', $school) }}" method="POST">@csrf @method('PUT')<p class="font-black">{{ $school->name }}</p><div class="mt-4 grid grid-cols-2 gap-3"><label class="block text-sm font-bold"><span class="mb-2 block">Hora de entrada</span><input class="{{ $fieldClass }}" name="attendance_entry_time" type="time" required value="{{ $school->attendance_entry_time?->format('H:i') ?? '08:00' }}"></label><label class="block text-sm font-bold"><span class="mb-2 block">Hora de salida</span><input class="{{ $fieldClass }}" name="attendance_exit_time" type="time" required value="{{ $school->attendance_exit_time?->format('H:i') ?? '14:00' }}"></label></div><label class="mt-3 block text-sm font-bold"><span class="mb-2 block">Margen antes de tardanza (minutos)</span><input class="{{ $fieldClass }}" name="attendance_late_grace_minutes" type="number" min="0" max="180" required value="{{ $school->attendance_late_grace_minutes }}"></label><label class="mt-3 block text-sm font-bold"><span class="mb-2 block">Tiempo mínimo antes de la salida (minutos)</span><input class="{{ $fieldClass }}" name="attendance_cooldown_minutes" type="number" min="1" max="120" required value="{{ $school->attendance_cooldown_minutes }}"></label><button class="mt-3 rounded-xl bg-teal-700 px-4 py-2 text-sm font-black text-white" type="submit">Guardar reglas</button></form>@endforeach</div>
</section>
@endif
